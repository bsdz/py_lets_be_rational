...
for i in range(TestCases):
    ...
    black = pylbr.black(F, K, sigma, T, q)
    iv = pylbr.implied_volatility_from_a_transformed_rational_guess(black, F, K, T, q)
    ivi = pylbr.implied_volatility_from_a_transformed_rational_guess_with_limited_iterations(
        black, F, K, T, q, N
    )

    x = _x[i]
    s = _s[i]
    nblack = pylbr.normalised_black(x, s, q)
    nblackcall = pylbr.normalised_black_call(x, s)
    niv = pylbr.normalised_implied_volatility_from_a_transformed_rational_guess(
        nblack, x, q
    )
    nivi = pylbr.normalised_implied_volatility_from_a_transformed_rational_guess_with_limited_iterations(
        nblack, x, q, N
    )
    vega = pylbr.normalised_vega(x, s)
    norm_cdf = pylbr.norm_cdf(z)

    data["input"].append(
        {"F": F, "K": K, "sigma": sigma, "T": T, "q": q, "z": z, "N": N, "x": x, "s": s}
    )
    data["output"].append(
        {
            "black": black,
